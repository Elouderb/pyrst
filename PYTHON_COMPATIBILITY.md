...
|---|---|---|---|---|
| `math` | 4/5 | 3 float consts (`pi`/`e`/`tau`) + `inf()`/`nan()` niladic externs + 33 `@extern` float wrappers + pure-pyrst `gcd`/`lcm`/`factorial`/`comb`/`perm`/`isqrt`/`modf`/`dist`/`prod` | Float-specialized (no generic numeric kind); `floor`/`ceil`/`trunc` return `float` not `int` (G6, deferred); `gcd`/`lcm`/`perm` lose CPython's variadic/defaulted-`k` shape (no `*args`); `inf`/`nan` are called as functions, not read as attributes; `int` is i64 — `factorial(21)+` overflows honestly instead of going bignum | ✅ dual-run (python3-reference: qualified `math.<fn>` calls, since `math` is `@extern`-backed) |
| `os` | 3/5 | `@extern`/`@crate("getrandom")` bindings: `getenv getcwd basename join dirname isfile isdir listdir mkdir remove read_file write_file walk stat stat_result getpid rename rmdir makedirs urandom sep linesep` | Flat only — no `os.path` submodule (deferred to G3); `os.environ` mutation deferred (G2); errors surface as a generic panic, not a typed `OSError`; `sep`/`linesep` are hardcoded POSIX values | ⚠️ pyrst-only — `@extern`/`@crate`-backed end to end (not real Python syntax), so no same-source dual run; each function was cross-checked against real python3 `os` individually |
...
| `operator` | 4/5 | 6 comparisons (`lt le gt ge eq ne`) generic over `T`; `itemgetter truth not_ contains_ concat` added this pass | `add`/`sub`/`mul`/`floordiv`/`mod_` stay int-specialized (generics-v2 bound inference doesn't cover `//`/`%`, and a by-value generic breaks the `reduce(add, ...)` idiom); `mod_`/`contains_` are forced renames (`mod`/`contains` collide with reserved/internal identifiers); `attrgetter`/`methodcaller` out of scope (need runtime reflection) | ⚠️ pyrst-only — `mod_`/`contains_` are forced renames that don't exist under those names in real CPython `operator`, so the same source can't run unmodified against python3 |
...
| `statistics` | 4/5 | `mean fmean median median_low median_high mode multimode quantiles variance stdev pvariance pstdev geometric_mean` over `list[float]` | Float-list only (no generic numeric bound yet); `quantiles`'s `n`/`method` are ordinary (not keyword-only) params, and pyrst has no plain-function keyword-call syntax, so the two languages can't share a call line once they're overridden; no `StatisticsError` — empty/degenerate input produces `inf`/`nan`/a runtime division instead of a typed exception | ✅ dual-run (default-argument path only, per the module header) |
| `string` | 4/5 | All 9 CPython constants (`ascii_lowercase … whitespace printable`) plus `capwords(s, sep=None)` | Only `Template`/`Formatter` (classes) remain deferred; `capwords`'s use of the shared `.capitalize()` builtin panics on a multi-byte-UTF-8-initial word (byte-slice bug in the shared string-builtin codegen, not string-module-specific) | ✅ dual-run |
| `bisect` | 4/5 | `bisect_left bisect_right bisect insort_left insort_right insort`, all with `lo=0, hi=None` | `key=` (CPython 3.10+) is deferred — needs a two-type-param `Callable[[T],K]` narrowing pattern not yet validated; explicit `hi=-1` intentionally does NOT reproduce CPython C-accelerator's undocumented `-1`-sentinel quirk (matches CPython's pure-Python fallback instead) | ✅ dual-run |
| `heapq` | 4/5 | `heappush heappop heapify heappushpop heapreplace nlargest nsmallest` | `nlargest`/`nsmallest` drop the `key=` callable param (no expressible `Any`-returning callable); `merge` is deferred (needs variadic `*iterables`, G4, plus `Iterator[T]`-as-parameter support) | ✅ dual-run |
| `collections` | 4/5 | `Counter` (function) + `most_common`/`counter_update`/`counter_subtract`/`counter_add`/`counter_sub`/`counter_and`/`counter_or`/`counter_total`/`counter_elements`; `deque` class with `rotate`/`extend`/`extendleft`/`count`/`remove`/`set_maxlen` | `Counter` is a function over `dict[T, int]`, not a dict-subclass class (`dict` has no operator-overload/method-attachment point in pyrst) — arithmetic is free functions (`counter_add(a, b)`), not `+`/`-` operators; tie-breaks and iteration order are by ascending key / most-common-first (deterministic), not CPython's first-insertion order, because pyrst dict iteration is unordered per-process | ⚠️ pyrst-only — `Counter` is a function (not CPython's class) and the `counter_*` free functions don't exist under those names in real CPython `collections` |
| `itertools` | 4/5 | LAZY generators: `count cycle repeat(x) chain islice takewhile dropwhile starmap accumulate(f) zip_longest` (infinite where CPython's are: `count`/`cycle`/bare `repeat`) | `chain`/`starmap` pairs are binary/2-tuple only (no `*args`); `accumulate`'s `f` is mandatory (no default-sum form, no `initial=`); `zip_longest`'s `fillvalue` is mandatory and must be positional; `groupby` is EAGER (`list[tuple[K, list[T]]]`), not a lazy grouping iterator | ⚠️ pyrst-only — pyrst has no keyword-argument call syntax at a free-function site, so `zip_longest`'s keyword-only `fillvalue` must be passed positionally, which real CPython's `zip_longest(*iterables, fillvalue=None)` signature interprets as an extra iterable instead |
| `textwrap` | 4/5 | `wrap fill shorten(text, width, placeholder=" [...]") indent(text, prefix) dedent(text)`, all keyword options now expressible as default args | Every argument must be passed **positionally** — named keyword-call syntax at a plain-function site is a **check-time error** (card d8a1ed83: a keyword argument at any site pyrst doesn't model is rejected uniformly at `check`/`build`, so it can no longer silently fall back to the default value); `drop_whitespace`/`expand_tabs`/`replace_whitespace`/`predicate` are not exposed as keywords | ✅ dual-run (CPython's own `textwrap`, calling positionally-compatible options only) |
...
| `json` | 4/5 | Pure-pyrst recursive-descent `loads`/serializer `dumps(v, indent=None, sort_keys=False)` over a tagged `JsonValue` class; surrogate-pair decoding | `JsonValue` is navigated via `.get(k)`/`.at(i)` methods, not `v["k"]`/`v[i]` subscripting (no dual `__getitem__` overload) — permanent, deliberate divergence; `load`/`dump` (file-object forms) deferred (no `file`-typed parameter spelling yet); `dumps` keyword args (`indent`/`sort_keys`) must be passed positionally, in order | ⚠️ pyrst-only — `JsonValue`/`.get`/`.at` don't exist under CPython's real `json` (which returns native `dict`/`list`), so the API shape can't run unmodified against python3; serialized-string behavior is cross-checked separately |
| `random` | 4/5 | `Random` class (seedable) with `random randint choice choices sample gauss normalvariate triangular betavariate getrandbits` and more | Backed by a 32-bit Numerical-Recipes **LCG**, not CPython's Mersenne Twister — same seed gives a **different** sequence than CPython (formula/shape matches, e.g. Box-Muller for `gauss`, but not numeric output); class-not-module (no mutable globals yet, G2, so no bare `random.random()`); `randbytes` deferred (needs `bytes`, G7) | ⚠️ pyrst-only — the LCG is a different generator than CPython's Mersenne Twister, so identical seeds produce different sequences; only pyrst-internal determinism (same seed ⇒ same sequence, run to run) is pinned |
